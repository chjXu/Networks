import torch
from torch._C import layout
import torch.nn as nn
from torch.nn.modules.activation import ReLU6
from torch.nn.modules.batchnorm import BatchNorm2d

# 作用就是将channel的个数，调整到离它最近的8的整数。这样做之后，对我们的硬件更加友好。
def _make_divisible(ch, divisor=8, min_ch=None):
    if min_ch is None:
        min_ch = divisor
    new_ch = max(min_ch, int(ch + divisor / 2) // divisor * divisor)
    # Make sure that round down does not go down by more than 10%.
    if new_ch < 0.9 * ch:
        new_ch += divisor
    return new_ch

# groups = 1时表示普通卷积
# groups = in_channel时，表示DW卷积
class ConvBNReLU(nn.Sequential):
    def __init__(self, in_channel, out_channnel, kernel_size=3, stride=1, groups=1):
        padding = (kernel_size-1) // 2
        super(ConvBNReLU, self).__init__(
            nn.Conv2d(in_channel, out_channnel, kernel_size, stride, padding, groups=groups, bias=False),
            nn.BatchNorm2d(out_channnel),
            nn.ReLU6(inplace=True)
        )

class InvertedResiduals(nn.Module):
    def __init__(self, in_channel, out_channel, stride, expand_ratio):
        super(InvertedResiduals, self).__init__()

        # 卷积核数量
        hidden_channel = in_channel * expand_ratio

        # 捷径分支
        self.use_shotcut = stride == 1 and in_channel == out_channel

        layers = []
        if expand_ratio != 1:
            #1x1 pointwise conv
            layers.append(ConvBNReLU(in_channel, hidden_channel, kernel_size=1))
        layers.extend([
            # 3x3 depthwise conv
            ConvBNReLU(hidden_channel, hidden_channel, stride=stride, groups=hidden_channel),

            #1x1 pointwise conv(linear)
            nn.Conv2d(hidden_channel, out_channel, kernel_size=1, bias=False),
            nn.BatchNorm2d(out_channel),
        ])

        self.conv = nn.Sequential(*layers)

    def forward(self, x):
        if self.use_shotcut:
            return x + self.conv(x)
        else: 
            return self.conv(x)



class MobileNetV2(nn.Module):
    def __init__(self, num_class = 1000, alpha = 1.0, round_nearest = 8):
        super(MobileNetV2, self).__init__()
        block = InvertedResiduals
        input_channel = _make_divisible(32 * alpha, round_nearest)
        last_channel = _make_divisible(1280 * alpha, round_nearest)

        inverted_residual_setting = [
            # t, c, n, s
            [1, 16, 1, 1],
            [6, 24, 2, 2],
            [6, 32, 3, 2],
            [6, 64, 4, 2],
            [6, 96, 3, 1],
            [6, 160, 3, 2],
            [6, 320, 1, 1],
        ]

        features = []
        # conv1
        features.append(ConvBNReLU(3, input_channel, stride=2))
    
        # bottleneck
        for t, c, n, s in inverted_residual_setting:
            output_channel = _make_divisible(c * alpha, round_nearest)
            for i in range(n):
                stride = s if i==0 else 1
                features.append(block(input_channel, output_channel, stride, expand_ratio=t))
                input_channel = output_channel

        features.append(ConvBNReLU(input_channel, last_channel, 1))

        self.features = nn.Sequential(*features)

        self.averagePooling = nn.AdaptiveAvgPool2d((1, 1))
        self.classify = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(last_channel, num_class)
        )

        for m in self.modules():
            if isinstance(m ,nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out')
                if m.bias is not None:
                    nn.init.zeros_(m.bias)
            elif isinstance(m, nn.BatchNorm2d):
                nn.init.ones_(m.weight)
                nn.init.zeros_(m.bias)
            elif isinstance(m, nn.Linear):
                nn.init.normal_(m.weight, 0, 0.01)
                nn.init.zeros_(m.bias)

    def forward(self, x):
        x = self.features(x)
        x = self.averagePooling(x)
        x = torch.flatten(x, 1)
        x = self.classify(x)
        return x
